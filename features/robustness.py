...
def volatility_window_sensitivity(
    ...
    Returns a DataFrame with sigma, price, and % deviation from baseline.
    """
    results = []
    baseline_price = None
...
def step_convergence(
    ...
    if step_grid is None:
        step_grid = [5, 10, 15, 20, 25, 30, 40, 50]

    rows = []
    for n in step_grid:
        params = calibrate(df, ticker, r=r, T=T, n=n)
        result = price_asian_call(params)
        rows.append({"n": n, "price": result["price"],
